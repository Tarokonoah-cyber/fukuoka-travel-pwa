...
export function Checklist({items,categories,storageKey,customPlaceholder="新增項目"}:{items:ChecklistItemData[];categories:string[];storageKey:StorageKey;customPlaceholder?:string}){
  ...
  return <div className="checklist-wrap"><ProgressBar value={all.filter(i=>state.checked.includes(i.id)).length} total={all.length}/>
    <form className="add-form" onSubmit={e=>{e.preventDefault();add()}}><label><span>自訂項目</span><input value={name} onChange={e=>setName(e.target.value)} placeholder={customPlaceholder}/></label><label><span>分類</span><select value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select></label><button type="submit">新增</button></form>

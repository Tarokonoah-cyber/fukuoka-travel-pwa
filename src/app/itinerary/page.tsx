...
export default function ItineraryPage(){const status=useTripStatus();return <div className="page-enter"><PageHeader eyebrow="5 DAYS / 4 NIGHTS" title="全部行程" description="每天留一點空白，走累了就休息。"/>
  ...
  <div className="all-days">{itinerary.map(day=><section id={`day-${day.day}`} key={day.day} className={status.phase==="during"&&status.day===day.day?"day-section current":"day-section"}>
    <header className="day-heading"><div><span>DAY {day.day} · {formatTripDate(day.date)}</span><h2>{day.title}</h2></div>{status.phase==="during"&&status.day===day.day&&<b>今天</b>}</header>
